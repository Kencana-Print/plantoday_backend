...
const getAuthSalesKode = (reqOrUser) => {
    const user = reqOrUser?.user || reqOrUser;
    return String(user?.sales_kode || "").trim();
};

const assertPenawaranOwnership = async ({ conn, nomor, user }) => {
    const managerRole = isManagerUser(user);
    const authSalesKode = getAuthSalesKode(user);
...
const ensurePenawaranSchema = async () => {
...
const getPenawaranDetail = async ({ managerRole, authSalesKode, nomor }) => {
    const ownerFilterSql = managerRole
        ? ""
        : "AND COALESCE(h.pen_sal_kode, '') = ?";
...
const createPenawaran = async ({
    ...
    try {
        ...
        if (!perusahaanKode) {
            return {
                status: 400,
                body: { success: false, message: "Perusahaan wajib diisi" },
            };
        }
        if (!customerKode) {
            return {
                status: 400,
                body: { success: false, message: "Customer wajib diisi" },
            };
        }
        ...
        return {
            ...
            body: {
                success: false,
                message:
                    err.sqlMessage || err.message || "Gagal membuat penawaran",
            },
        };
    } finally {
        if (lockState?.acquired) {
...
const getMasterPerusahaan = async (search = "") => {
...
const getMasterCustomer = async (search = "") => {
    const s = String(search || "").trim();
    const like = `%${s}%`;
...
const getMasterSales = async (search = "") => {
    const s = String(search || "").trim();
    const like = `%${s}%`;
...
const getMasterPenawaranNomor = async (search = "") => {
    const s = String(search || "").trim();
    const like = `%${s}%`;
...
const getMasterPermintaanHargaForPenawaran = async ({
    effectiveSalesKode,
    ...
    if (selectedNomor) {
        ...
        if (detailRows?.length) {
            ...
            if (
                referenceCustomerKode &&
                String(d.customer_kode || "").trim() !== referenceCustomerKode
            ) {
                selectedError = {
                    status: 409,
                    message:
            ...
            } else if (!isPermintaanStatusSelesai(d.status)) {
                selectedError = {
                    status: 409,
                    message:
...
const updatePenawaranStatusDetail = async ({
    ...
    try {
        ...
        const ownership = await assertPenawaranOwnership({ conn, nomor, user });
        ...
        const authUserNama = String(user?.nama || "").trim();
        const userModified = authUserNama || "MOBILE";
...
const getMasterPenawaranBatal = async () => {
    const [rows] = await db.query(
        `
        SELECT
...
const getMasterPenawaranConfirm = async () => {
    const [rows] = await db.query(
        `
        SELECT
...
const requestApprovalPerubahan = async ({
    ...
    try {
        ...
        const userCreate = (authUserNama || authUserId || "MOBILE")
            .trim()
            .toUpperCase();
